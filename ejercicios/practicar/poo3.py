...
class Coche(): # La primera latra de la clase siempre en mayúscula
	...
	def __init__(self): # Esta función es un constructor para dar a nuestra clase un estado inicial claro (el objeto va a tener ese estado inicial)
	...
	def arrancarCoche(self,arrancamos): # Self hace referencia al propio objeto que pertenece a la clase y solicitamos un parámetro que sea True or False
	...
	def __chequeo_interno(self): # Creamos un método para chequear nuestro coche,con los __ lo que hacemos es encapsular el método para que no se pueda acceder fuera de la clase.
...
print(miCoche.arrancarCoche(True)) # Hacemos una llamada al metodo, ese método recibe por parámetro es el propio objeto (self), miCoche se almacena en el self de el método.Tiene que ir con print porque el return de la función devuelve string(arranca el coche porque le pasamos True)
