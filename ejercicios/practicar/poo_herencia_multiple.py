...
class Moto(Vehiculos): # Creamos una clase moto y dentro de los paréntesis va la clase de la que queremos heredar
	...
	def caballito(self): #Creamos un método para la clase moto;tiene un comportamiento más que un vehículo no tiene; tiene los métodos heredados mas el suyo propio.
		self.hcaballito="Voy haciendo un caballito"


	# A continuación vamos a hacer lo que se conoce como sobreescritura de métodos:
	def estado(self): # copiamos el estado del metodo de la clase padre(con esto sobreescribimos el estado inicial y ya tiene su propio estado) y añadimos la propiedad o comportamiento propio de esta clase.
...
miMoto.caballito() # LLamamos al metodo de caballito y entra el método con lo cual hcaballito (cadena de caracteres vacías) toma el valor de dentro del método(self.hcaballito).
...
#HERENCIA MÚLTIPLE#
# A continuación vamos a heredar de dos clases(se puede heredar de varias clases). *****Siempre Siempre**** hereda del constructor de la primera clase que nombres en el paréntesis
class BicicletaElectrica(Vehiculos,VElectricos): #esta clase hereda de vehiculos y Velectricos
# Class BicicletaElectrica(VElectricos,Vehiculos): #Si desbloqueas esta linea y comentamos la anterior heredará del constructor VElectricos y va a dar error porque VElectricos no solicita parámetro
	pass
